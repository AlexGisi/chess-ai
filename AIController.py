...
class AIController:
    ...
    def heuristic(self, board: chess.Board) -> float:
        ...
        return AIController.RESULT_MAP[result] if result in AIController.RESULT_MAP \
            ...
            [AIController.PIECE_MAP[char] if char in AIController.PIECE_MAP else 0 for char in board.board_fen()])

    # TODO: seems to maximize for white always, not making easy plays for black.
    def minimax(self, board: chess.Board, depth: int, maximizingPlayer: bool, alpha: float, beta: float) -> float:
    ...
    def get_move(self, board: chess.Board, player: bool) -> str:
        self.positionsEvaluated = 0
        timer = time()
