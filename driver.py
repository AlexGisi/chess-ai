...
class Driver:
    ...
    def run_game(self):
        while self.ci.get_result() == "*":
            turn = self.ci.get_turn()
            print(f"\n{self.ci.board}\n{'white to move' if turn else 'black to move'}\n")
            try:
                if turn:  # white to move
                    self.ci.move(self.cont1.get_move(self.ci.board, True))
                else:
                    self.ci.move(self.cont2.get_move(self.ci.board, False))
            except ValueError:
                print("Invalid move, try again\n")

        print(f"\n{self.ci.board}\n{'white to move' if turn else 'black to move'}\n")
        print(self.ci.get_result())
